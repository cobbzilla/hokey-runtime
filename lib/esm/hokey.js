...
export const getLang = (loc) => (loc.includes("_") ? loc.substring(0, loc.indexOf("_")) : loc);
export class Hokey {
    constructor(ALL_MESSAGES, FALLBACK_DEFAULT_LOCALE, SELECTABLE_LOCALES) {
        this.stopWords = () => {
    ...
    localesList(account, browserLocale, anonLocale) {
        ...
        return this.SELECTABLE_LOCALES.map((loc) => {
    ...
    findFirstLocaleNameMatch(locales, defaultLocale) {
        ...
        if (this.ALL_MESSAGES[this.FALLBACK_DEFAULT_LOCALE])
            return this.FALLBACK_DEFAULT_LOCALE;
